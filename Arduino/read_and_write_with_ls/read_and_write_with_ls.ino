...
**************************************************************************/
void loop() {
  car.RC_or_CC = digitalRead(car.SWITCH_PIN);
  ...
  if (dt > 50) {
    ...
    if(car.RC_or_CC ==HIGH){
      car.writeToActuatorsRC(rc_inputs.motor,rc_inputs.servo);
    }
...
float Car::saturateServoRC(float x) {
  if (x  == noAction ) {
    return THETA_CENTER;
  }
  if (x  >  THETA_MAX) {
...
void Car::writeToActuatorsRC(uint8_t rc_inputs_motor,uint8_t rc_inputs_servo) {
  float motorCMD = saturateMotor(rc_inputs_motor);
  float servoCMD = saturateServoRC(rc_inputs_servo);
  motor.writeMicroseconds( (uint16_t) (1500.0 + (motorCMD-90.0)*1000.0/180.0) );
  steering.write(servoCMD);
}
